...
uint explode(
        ...
        void*  param)
{
    TDcmpStruct * pWork = cast(TDcmpStruct *)work_buf;
    ...
    pWork.param      = param;
    pWork.in_pos     = pWork.in_buff.sizeof;
    pWork.in_bytes   = pWork.read_buf(cast(ubyte *)pWork.in_buff, &pWork.in_pos, pWork.param);
    if(pWork.in_bytes <= 4)
        return CMP_BAD_DATA;
    ...
    if(pWork.ctype != CMP_BINARY)
    {
        ...
        pWork.ChBitsAsc[] = ChBitsAsc[];
        GenAscTabs(pWork);
    }

    pWork.LenBits[] = LenBits[];
    GenDecodeTabs(pWork.LengthCodes.ptr, LenCode.ptr, pWork.LenBits.ptr, pWork.LenBits.sizeof);
    pWork.ExLenBits[] = ExLenBits[];
    pWork.LenBase[] = LenBase[];
    pWork.DistBits[] = DistBits[];
    GenDecodeTabs(pWork.DistPosCodes.ptr, DistCode.ptr, pWork.DistBits.ptr, pWork.DistBits.sizeof);
...
enum PKDCL_OK                    = 0;       
enum PKDCL_STREAM_END            = 1;   // All data from the input stream is read
enum PKDCL_NEED_DICT             = 2;   // Need more data (dictionary)
enum PKDCL_CONTINUE             = 10;   // Internal flag, not returned to user
enum PKDCL_GET_INPUT            = 11;   // Internal flag, not returned to user

string CopyrightPkware = "PKWARE Data Compression Library for Win32\r\n"
...
static void GenDecodeTabs(
    ubyte * positions,          // [out] Table of positions
    const ubyte * start_indexes,      // [in] Table of start indexes
    const ubyte * length_bits,        // [in] Table of lengths. Each length is stored as number of bits
...
{
    auto pChCodeAsc = &ChCodeAsc[0xFF];
    uint  acc, add;
    ushort count;

    for(count = 0x00FF; pChCodeAsc >= ChCodeAsc.ptr; pChCodeAsc--, count--)
    {
        ubyte * pChBitsAsc = pWork.ChBitsAsc.ptr + count;
        ubyte bits_asc = *pChBitsAsc;
        ...
        if(bits_asc <= 8)
        {
            ...
            do
            {
                pWork.offs2C34[acc] = cast(ubyte)count;
                acc += add;
            }
            while(acc < 0x100);
        }
        else if((acc = (*pChCodeAsc & 0xFF)) != 0)
        {
            ...
            if(*pChCodeAsc & 0x3F)
            {
                ...
                *pChBitsAsc = bits_asc;
                ...
                do
                {
                    pWork.offs2D34[acc] = cast(ubyte)count;
                    acc += add;
                }
                while(acc < 0x100);
            }
            else
            {
                ...
                *pChBitsAsc = bits_asc;
                ...
                do
                {
                    pWork.offs2E34[acc] = cast(ubyte)count;
                    acc += add;
                }
                while(acc < 0x80);
            }
        ...
        else
        {
            ...
            *pChBitsAsc = bits_asc;
            ...
            do
            {
                pWork.offs2EB4[acc] = cast(ubyte)count;
                acc += add;
            }
            while(acc < 0x100);
        }
...
static int WasteBits(TDcmpStruct * pWork, uint nBits)
{
    ...
    {
        pWork.in_pos = pWork.in_buff.sizeof;
        if((pWork.in_bytes = pWork.read_buf(cast(ubyte *)pWork.in_buff, &pWork.in_pos, pWork.param)) == 0)
...
static uint DecodeLit(TDcmpStruct * pWork)
{
    uint extra_length_bits;    // Number of bits of extra literal length
    uint length_code;          // Length code
    uint value;
    ...
    if(pWork.bit_buff & 1)
    {
        ...
        {
            uint extra_length = pWork.bit_buff & ((1 << extra_length_bits) - 1);
...
static uint DecodeDist(TDcmpStruct * pWork, uint rep_length)
{
    uint dist_pos_code;            // Distance position code
    uint dist_pos_bits;            // Number of bits of distance position
    uint distance;                 // Distance position
...
static uint Expand(TDcmpStruct * pWork)
{
    uint next_literal;         // Literal decoded from the compressed data
    uint result;               // Value to be returned
    uint copyBytes;             // Number of bytes to copy to the output buffer
    ...
    while((result = next_literal = DecodeLit(pWork)) < 0x305)
    {
        ...
        if(next_literal >= 0x100)
        {
            ubyte * source;
            ubyte * target;
            uint rep_length;       // Length of the repetition, in bytes
            uint minus_dist;       // Backward distance to the repetition, relative to the current buffer position
        ...
        {
            pWork.out_buff[pWork.outputPos++] = cast(ubyte)next_literal;
        }
        ...
        if(pWork.outputPos >= 0x2000)
        {
            ...
            copyBytes = 0x1000;
            pWork.write_buf(cast(ubyte *)&pWork.out_buff[0x1000], &copyBytes, pWork.param);
            ...
            pWork.out_buff[0 .. pWork.outputPos - 0x1000] = pWork.out_buff[0x1000 .. pWork.outputPos - 0x1000];
