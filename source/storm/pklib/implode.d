...
/*****************************************************************************/
module storm.pklib.implode;

import storm.pklib.pklib;

uint implode(
    uint function(ubyte *buf, uint *size, void *param) read_buf,
    void function(ubyte *buf, uint *size, void *param) write_buf,
    ubyte* work_buf,
    void*  param,
    uint *type,
    uint *dsize)
{
    TCmpStruct * pWork = cast(TCmpStruct *)work_buf;
    uint nChCode;
    uint nCount;
    uint i;
    int nCount2;
    ...
    switch(*dsize)
    {
        case CMP_IMPLODE_DICT_SIZE3:    // 0x1000 bytes
            ...
            // No break here !!!
            goto case CMP_IMPLODE_DICT_SIZE2;
            
        case CMP_IMPLODE_DICT_SIZE2:    // 0x800 bytes
            pWork.dsize_bits++;
            pWork.dsize_mask |= 0x10;
            // No break here !!!
            goto case CMP_IMPLODE_DICT_SIZE1;
    ...
    switch(*type)
    {
        case CMP_BINARY: // We will compress data with binary compression type
            for(nChCode = 0, nCount = 0; nCount < 0x100; nCount++)
            {
                ...
                pWork.nChCodes[nCount] = cast(ushort)nChCode;
        ...
        case CMP_ASCII: // We will compress data with ASCII compression type
            ...
            {
                pWork.nChBits[nCount]  = cast(ubyte)(ChBitsAsc[nCount] + 1);
                pWork.nChCodes[nCount] = cast(ushort)(ChCodeAsc[nCount] * 2);
            }
            break;
    ...
    for(i = 0; i < 0x10; i++)
    {
        if(1 << ExLenBits[i])
        {
            ...
            {
                pWork.nChBits[nCount]  = cast(ubyte)(ExLenBits[i] + LenBits[i] + 1);
                pWork.nChCodes[nCount] = cast(ushort)((nCount2 << (LenBits[i] + 1)) | ((LenCode[i] & 0xFFFF00FF) * 2) | 1);
                nCount++;
            }
        }
    }
    ...
    pWork.dist_codes[0 .. DistCode.length] = DistCode[];
    pWork.dist_bits[0 .. DistBits.length] = DistBits[];
    WriteCmpData(pWork);
    return CMP_NO_ERROR;
}
...
enum MAX_REP_LENGTH = 0x204;            // The longest allowed repetition

//-----------------------------------------------------------------------------
// Tables

immutable ubyte[] DistBits = 
[
...
immutable ubyte[] ExLenBits =
[
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08
];

immutable ubyte[] LenBits =
[
    0x03, 0x02, 0x03, 0x03, 0x04, 0x04, 0x04, 0x05, 0x05, 0x05, 0x05, 0x06, 0x06, 0x06, 0x07, 0x07
];

immutable ubyte[] LenCode =
[
    0x05, 0x03, 0x01, 0x06, 0x0A, 0x02, 0x0C, 0x14, 0x04, 0x18, 0x08, 0x30, 0x10, 0x20, 0x40, 0x00
];

immutable ubyte[] ChBitsAsc =
[
...
uint BYTE_PAIR_HASH(ubyte* buffer)   {return (buffer[0] * 4) + (buffer[1] * 5);}
...
static void SortBuffer(TCmpStruct * pWork, ubyte * buffer_begin, ubyte * buffer_end)
{
    ushort * phash_to_index;
    ubyte  * buffer_ptr;
    ushort total_sum = 0;
    uint byte_pair_hash;           // Hash value of the byte pair
    ushort byte_pair_offs;          // Offset of the byte pair, relative to "work_buff"

    // Zero the entire "phash_to_index" table
    pWork.phash_to_index[] = 0;
    ...
    for(phash_to_index = pWork.phash_to_index.ptr; phash_to_index < &pWork.phash_to_index_end; phash_to_index++)
    {
        total_sum = cast(ushort)(total_sum + phash_to_index[0]);
    ...
    for(buffer_end--; buffer_end >= buffer_begin; buffer_end--)
    {
        ...
        byte_pair_offs = cast(ushort)(buffer_end - pWork.work_buff.ptr);
...
static void FlushBuf(TCmpStruct * pWork)
{
    ...
    pWork.write_buf(pWork.out_buff.ptr, &size, pWork.param);
...
static void OutputBits(TCmpStruct * pWork, uint nbits, uint bit_buff)
{
    uint out_bits;
    ...
    pWork.out_buff[pWork.out_bytes] |= cast(ubyte)(bit_buff << out_bits);
    ...
    if(pWork.out_bits > 8)
    {
        ...
        pWork.out_buff[pWork.out_bytes] = cast(ubyte)bit_buff;
        pWork.out_bits &= 7;
    }
    else
    {
...
// to pWork structure.
static uint FindRep(TCmpStruct * pWork, ubyte * input_data)
{
    ushort * phash_to_index;            // Pointer into pWork.phash_to_index table
    ushort * phash_offs;                // Pointer to the table containing offsets of each PAIR_HASH
    ubyte * repetition_limit;           // An eventual repetition must be at position below this pointer
    ubyte * prev_repetition;            // Pointer to the previous occurence of the current PAIR_HASH
    ubyte * prev_rep_end;               // End of the previous repetition
    ubyte * input_data_ptr;
    ushort phash_offs_index;            // Index to the table with PAIR_HASH positions
    ushort min_phash_offs;              // The lowest allowed hash offset
    ushort offs_in_rep;                 // Offset within found repetition
    uint equal_byte_count;              // Number of bytes that are equal to the previous occurence
    uint rep_length = 1;                // Length of the found repetition
    uint rep_length2;                   // Secondary repetition
    ubyte pre_last_byte;                // Last but one byte from a repetion
    ushort di_val;

    // Calculate the previous position of the PAIR_HASH
    phash_to_index   = pWork.phash_to_index.ptr + BYTE_PAIR_HASH(input_data);
    min_phash_offs   = cast(ushort)((input_data - pWork.work_buff.ptr) - pWork.dsize_bytes + 1);
    phash_offs_index = phash_to_index[0];

    // If the PAIR_HASH offset is below the limit, find a next one
    phash_offs = pWork.phash_offs.ptr + phash_offs_index;
    ...
    phash_offs = pWork.phash_offs.ptr + phash_offs_index;
    prev_repetition = pWork.work_buff.ptr + phash_offs[0];
    ...
    for(;;)
    {
        ...
        if(*input_data_ptr == *prev_repetition && input_data_ptr[rep_length-1] == prev_repetition[rep_length-1])
        {
            ...
            if(equal_byte_count >= rep_length)
            {
                ...
                pWork.distance = cast(uint)(input_data - prev_repetition + equal_byte_count - 1);
    ...
    phash_offs = pWork.phash_offs.ptr + phash_offs_index;
    if(pWork.work_buff.ptr + phash_offs[1] >= repetition_limit)
        return rep_length;
    ...
    for(;;)
    {
        ...
        do
        {
            phash_offs++;
            phash_offs_index++;
            prev_repetition = pWork.work_buff.ptr + *phash_offs;
            if(prev_repetition >= repetition_limit)
                return rep_length;
        }
        ...
        if(rep_length2 >= rep_length)
        {
            ...
            pWork.distance = cast(uint)(input_data - prev_repetition - 1);
...
{
    ubyte * input_data_end;         // Pointer to the end of the input data
    ubyte * input_data = pWork.work_buff.ptr + pWork.dsize_bytes + 0x204;
    uint input_data_ended = 0;      // If 1, then all data from the input stream have been already loaded
    uint save_rep_length;           // Saved length of current repetition
    uint save_distance = 0;         // Saved distance of current repetition
    uint rep_length;                // Length of the found repetition
    uint phase = 0;                 // 

    // Store the compression type and dictionary size
    pWork.out_buff[0] = cast(ubyte)pWork.ctype;
    pWork.out_buff[1] = cast(ubyte)pWork.dsize_bits;
    pWork.out_bytes = 2;
    ...
    while(input_data_ended == 0)
    {
        ...
        {
            bytes_loaded = pWork.read_buf(cast(ubyte *)pWork.work_buff + pWork.dsize_bytes + 0x204 + total_loaded,
                                                  &bytes_to_load,
                                                   pWork.param);
            if(bytes_loaded == 0)
            {
        ...
        input_data_end = pWork.work_buff.ptr + pWork.dsize_bytes + total_loaded;
        if(input_data_ended)
        ...
        while(input_data < input_data_end)
        {
            ...
            while(rep_length != 0)
            {
                ...
                if(input_data_ended && input_data + rep_length > input_data_end)
                {
                    ...
                    rep_length = cast(uint)(input_data_end - input_data);
                    if(rep_length < 2)
                        break;
...
_00402252:;
        ...
        {
            input_data -= 0x1000;
            pWork.work_buff[0 .. pWork.dsize_bytes + 0x204] = pWork.work_buff[0x1000 .. pWork.dsize_bytes + 0x204];
        }
    }
...
__Exit:
    ...
    if(pWork.out_bits != 0)
        pWork.out_bytes++;
    pWork.write_buf(pWork.out_buff.ptr, &pWork.out_bytes, pWork.param);
    return;
}